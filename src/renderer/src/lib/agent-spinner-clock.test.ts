...
function stubDom(visibilityState: () => DocumentVisibilityState): {
  ...
  fireMainWindowVisibility: (visible: boolean) => void
} {
  let mainVisibilityCallback: ((visible: boolean) => void) | null = null
    ...
    api: {
      ui: {
        onWindowVisibilityChanged: vi.fn((callback: (visible: boolean) => void) => {
          mainVisibilityCallback = callback
          return () => {
            mainVisibilityCallback = null
          }
        })
      }
    }
  ...
  return {
    ...
    fireMainWindowVisibility: (visible: boolean) => {
      mainVisibilityCallback?.(visible)
    }
  }
}
...
describe('agent-spinner-clock', () => {
  ...
  it('stops when main reports the window hidden even though visibilityState stays visible', () => {
    // macOS: setBackgroundThrottling(false) pins document.visibilityState to
    // 'visible' for a hidden window, so the main-process relay is the only
    // trustworthy hidden signal there.
    const { fireMainWindowVisibility } = stubDom(() => 'visible')
    const el = makeElement()
    const unregister = registerAgentSpinnerElement(el)

    fireMainWindowVisibility(false)
    const frozen = (el as unknown as FakeElement).style.transform
    vi.advanceTimersByTime(AGENT_SPINNER_TICK_MS * 10)
    expect((el as unknown as FakeElement).style.transform).toBe(frozen)

    fireMainWindowVisibility(true)
    expect((el as unknown as FakeElement).style.transform).not.toBe(frozen)
